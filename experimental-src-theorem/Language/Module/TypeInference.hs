...
infer :: (MonadError GalcError m, MonadFresh [String] String m, MonadOr m) 
      => ModuleS -> m Module
infer (ModuleS nm laws' gcs' defs' thrs') = do
  laws'' <- mapM L.infer laws'
  gcs''  <- mapM R.infer gcs'
  defs'' <- mapM R.infer defs'
  thrs'' <- mapM L.inferT thrs'
  return $ Module {
    name = nm, 
    laws = Map.fromList . map (\x -> (LS.getName x, x)) $ laws'' ++ thrs'', 
    gcs = Map.fromList . map (\x -> (gcName x, x)) $ gcs'', 
    definitions = Map.fromList . map (\x -> (defName x, x)) $ defs'' }

-------------------------------------------------------------------------------

--processTheorem :: (MonadError GalcError m, MonadFresh [String] String m, MonadOr m)
--               => Theorem -> m Law
--processTheorem (
