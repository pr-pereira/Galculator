...
refreshT :: MonadFresh [String] String m => Theorem -> m Theorem
refreshT (EquivT p n s1 s2 sp) = auxT (EquivT p n) s1 s2 sp
refreshT (ImplT p n s1 s2 sp)  = auxT (ImplT p n) s1 s2 sp

-------------------------------------------------------------------------------

auxT :: MonadFresh [String] String m 
    => (S -> S -> Step -> Theorem) -> S -> S -> Step -> m Theorem
auxT constr s1 s2 sp = let
    (rvar1,tvar1) = R.collect s1
    (rvar2,tvar2) = R.collect s2
    rvar = nub $ rvar1 ++ rvar2
    tvar = nub $ tvar1 ++ tvar2
