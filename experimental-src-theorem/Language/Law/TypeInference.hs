...
inferT :: (MonadOr m, MonadError GalcError m, MonadFresh [String] String m) 
      => Theorem -> m Law
inferT s = evalStateT (inferT' s) emptyEnv

-------------------------------------------------------------------------------
inferT' :: (MonadOr m, MonadError GalcError m, MonadState R.InfEnv m, MonadFresh [String] String m) 
       => Theorem -> m Law
-------------------------------------------------------------------------------
inferT' a@(EquivT _ ident r1a r2a _) = R.catchInfer a $ do
  (Exists tr1 r1, Exists tr2 r2, unif, _) <- R.inferBin r1a r2a
  constr <- unify $ [tr1 :=: tr2] ++ unif
  Hide t1' <- typeRewrite constr tr1
  r1' <- rCast constr t1' r1
  ...
  return $ EQUIV (Meta {name = ident, ruleType = Nothing}) t1' r1' r2'
-------------------------------------------------------------------------------
inferT' a@(ImplT _ ident r1a r2a _) = R.catchInfer a $ do
  (Exists tr1 r1, Exists tr2 r2, unif, _) <- R.inferBin r1a r2a
  constr <- unify $ [tr1 :=: tr2] ++ unif
  Hide t1' <- typeRewrite constr tr1
  r1' <- rCast constr t1' r1
