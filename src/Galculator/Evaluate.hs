...
{-# LANGUAGE TypeOperators, Rank2Types, RankNTypes, PatternSignatures, ImpredicativeTypes #-}
-- {-# LANGUAGE TypeOperators, Rank2Types, RankNTypes, PatternSignatures, ImpredicativeTypes, LiberalTypeSynonyms, PatternSynonyms, QuantifiedConstraints #-}
{-# OPTIONS_GHC -Wall #-}
...
combBin c1 comb c2 = evalCombinator c1 >>= \c1' -> evalCombinator c2 >>= \c2' -> return (comb c1' c2')
{-do
...
combUni comb c = evalCombinator c >>= return . comb
  {-
  c' <- evalCombinator c
  let rc = comb c' 
  return rc-}
...
evalCombinator (Rule r) = 
  case r of
    Inv drv -> evalDerivation drv >>= return . getRuleInv
    {-do 
      lw <- evalDerivation drv
      let rl = getRuleInv lw
      --let (rl::Rule) = getRuleInv lw
      return rl-}
    _ -> evalDerivation r >>= return . getRule
    {-
    do
      lw::Law <- evalDerivation r
