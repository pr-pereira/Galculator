...
{-# LANGUAGE PatternSignatures, FlexibleContexts, FlexibleInstances, UndecidableInstances #-}
{-# OPTIONS_GHC -Wall #-}
...
-- prp
-- readModule :: (MonadIO m, MonadError GalcError m) => String -> m String
readModule mdl = do
  res <- liftIO $ tryIOError (readFile $ mdl ++ ".gal")
  case res of
    Left e -> throwError (ModuleFileError mdl)
    Right r -> return r
-------------------------------------------------------------------------------
