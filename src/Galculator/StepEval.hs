...
-------------------------------------------------------------------------------
stepEval (Comb comb) = 
 inMode [ProofMode, IndirectProofMode] $ updateProof $ \prf -> 
   evalCombinator comb >>= \rl -> 
     getCurExpr >>= \(Exists t r) -> 
       return $ maybe prf (\(expr, proof) -> 
...
stepEval (Indirect ord'') = inMode [ProofMode] $ (updateProof $ \prf -> do
  ...
  where
    ...
    auxLeft (Exists te e) (Exists to o) = do
      ...
      return $ Exists (Rel t2' t1') (COMP t2' (ORD o') e')
...
stepEval IndirectEnd = inMode [IndirectProofMode] $ (updateProof $ \prf -> do
  ...
  where
    ...
    auxLeft (Exists te e) (Exists to o) = do
      ...
      rConstr <- rMatch (COMP t2' (ORD o') r) e'
      -- rConstr <- rMatch (COMP t2' (ORD o') (FUN r)) e'
      let r' = rSubst rConstr (Rel t2' t1') r
      --let r' = rSubst rConstr (Fun t2' t1') r
      return $ Exists (Rel t2' t1') r'
    ...
    auxRight (Exists te e) (Exists to o) = do
      ...
      r <- getFreshLift (Var . ('r':))
      rConstr <- rMatch (COMP t2' (CONV r) (ORD o')) e'
