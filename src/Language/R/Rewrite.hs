...
{-# LANGUAGE GADTs, Rank2Types, ImpredicativeTypes #-}
{-# OPTIONS_GHC -Wall #-}
...
rec :: Monad m => (GenericM m -> GenericM m) -> GenericM m
rec f = f (rec f)

manyR :: MonadOr m => GenericM m -> GenericM m
manyR r = rec (\f -> ((r >>> f) |<| nop))
...
onceR :: MonadOr m => GenericM m -> GenericM m
onceR r = rec (\f -> r |<| one f)

-------------------------------------------------------------------------------

everywhere :: Monad m => GenericM m -> GenericM m
--everywhere f = f >>> all (everywhere f)
everywhere = everywhereR

everywhereR :: Monad m => GenericM m -> GenericM m
everywhereR r = rec  (\f -> r >>> all f)
...
everywhereR' :: Monad m => GenericM m -> GenericM m
everywhereR' r = rec (\f -> all f >>> r)

-------------------------------------------------------------------------------

innermost :: MonadOr m => GenericM m -> GenericM m
--innermost f = all (innermost f) >>> try (f >>> innermost f)
innermost = innermostR

innermostR :: MonadOr m => GenericM m -> GenericM m
innermostR r = rec (\f -> all f >>> try (r >>> f))
