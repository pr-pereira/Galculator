...
--instance Except GalcError where
...
instance Semigroup GalcError where
  (<>) = undefined
instance Monoid GalcError where
  mempty = undefined
-------------------------------------------------------------------------------

instance MonadPlus (Either GalcError) where
  mzero = Left UnknownError
  mplus (Left _) m2 = m2
  mplus a _ = a
  
instance Alternative (Either GalcError) where
