...
infer' :: (
       ...
       => S -> m RType
-------------------------------------------------------------------------------
infer' a@(EqualS _ r1a r2a) = catchInfer a $ do
  (Exists tr1 r1, Exists tr2 r2, unif, _) <- inferBin r1a r2a
  constr <- unify $ [tr1 :=: tr2] ++ unif
  Hide t1' <- typeRewriteE constr tr1
  r1' <- rCast constr t1' r1
  r2' <- rCast constr t1' r2
  return $ Exists (Expr t1') (EQUAL r1' r2')
-------------------------------------------------------------------------------
infer' a@(LessS _ r1a r2a) = catchInfer a $ do
  (Exists tr1 r1, Exists tr2 r2, unif, _) <- inferBin r1a r2a
  constr <- unify $ [tr1 :=: tr2] ++ unif
  Hide t1' <- typeRewriteE constr tr1
  r1' <- rCast constr t1' r1
  r2' <- rCast constr t1' r2
  return $ Exists (Expr t1') (LESS r1' r2')
-------------------------------------------------------------------------------
