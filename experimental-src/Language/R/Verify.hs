...
-- It is assumed that there are no repeated definition in the external
-- environment
verify :: MonadError GalcError m => ExtEnv -> S -> m S
verify extEnv s = runReaderT (replaceDefs s) extEnv
...
replaceDefs :: (MonadError GalcError m, MonadReader ExtEnv m)
            => S -> m S
replaceDefs (EqualS p s1 s2) = do
  s1' <- replaceDefs s1
  s2' <- replaceDefs s2
  return $ EqualS p s1' s2'
replaceDefs (LessS p s1 s2) = do
  s1' <- replaceDefs s1
  s2' <- replaceDefs s2
  return $ LessS p s1' s2'
replaceDefs (RefS p n) = do -- TODO position in the substitution
  extEnv <- ask
  maybe (throwError $ ReferenceError n) (return . RefExtS p) .
         lookupEnv n $ extEnv
