...
-------------------------------------------------------------------------------
-- TODO: Return the dictionary generated by the substitutions
-- Equal variable identifiers should be replaced by the same variable 
-- identifier. This means removing the repetitions (a set may be used).
refresh :: MonadFresh [String] String m => S -> m S
refresh s = do
    let (rvar,tvar) = collect s
    newVars <- refreshVar . nub $ rvar
    newTypes <- refreshType . nub $ tvar
...
collect (EqualS _ s1 s2) = let
   (i1, tb1) = collect s1
   (i2, tb2) = collect s2
  in (i1++i2, tb1++tb2)
collect (LessS _ s1 s2) = let
   (i1, tb1) = collect s1
   (i2, tb2) = collect s2
  in (i1++i2, tb1++tb2)
...
replace (EqualS p s1 s2) = do
  s1' <- replace s1
  s2' <- replace s2
  return $ EqualS p s1' s2'
replace (LessS p s1 s2) = do
  s1' <- replace s1
  s2' <- replace s2
  return $ EqualS p s1' s2'
